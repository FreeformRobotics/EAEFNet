...
class IDAM(nn.Module):
    ...
    def forward(self, RGB, T):
        ...
        New_RGB = New_RGB * attention_vector_l + New_RGB
        New_T = New_T * attention_vector_r + New_T
        ##########################################################################
        return New_RGB, New_T, New_fuse
...
class FA_encoder(nn.Module):
    def __init__(self,dropout_rate: float = .09):
        ...
        self.idam_0 = IDAM(64)
        self.idam_1 = IDAM(64)
        self.idam_2 = IDAM(128)
        self.idam_3 = IDAM(256)
        self.idam_4 = IDAM(512)
        ########  RGB ENCODER  ########
    ...
    def forward(self, rgb,thermal):
        ...
        ######################################################################
        rgb, thermal,fuse = self.idam_0(rgb, thermal)
        ######################################################################
        # layer1
        ######################################################################
        ...
        rgb_1, thermal_1,fuse_1 = self.idam_1(rgb_1, thermal_1)
        ######################################################################
        # layer2
        ######################################################################
        ...
        rgb_2_1, thermal_2,fuse_2_1 = self.idam_2(rgb_2_1, thermal_2)
        ######################################################################
        # layer3
        ######################################################################
        ...
        rgb_3_1, thermal_3_1,fuse_3_1 = self.idam_3(rgb_3_1, thermal_3_1)
        ######################################################################
        # layer4
        ######################################################################
        ...
        rgb_4_1, thermal_4_1,fuse_4_1 = self.idam_4(rgb_4_1, thermal_4_1)
        ######################################################################
...
class Turbo_decoder(nn.Module):
    def __init__(self,n_class=1, channel=64):
        super(Turbo_decoder, self).__init__()
        ########  DECODER  ########
        self.rfb2_1 = GCM(128, channel)
        self.rfb3_1 = GCM(256, channel)
        self.rfb4_1 = GCM(512, channel)
        self.rfb0_2 = GCM(64, channel)
        self.rfb1_2 = GCM(64, channel)
        self.rfb5_2 = GCM(128, channel)
...
class FATNet_pp(nn.Module):
    ...
    def forward(self,rgb,thermal):
        encoder_input = self.FA_encoder(rgb,thermal)
...
def unit_test():
    ...
    thermal = torch.randn(num_minibatch, 3, 480, 640).cuda(0)
    rtf_net = FATNet_pp(1).cuda(0)
