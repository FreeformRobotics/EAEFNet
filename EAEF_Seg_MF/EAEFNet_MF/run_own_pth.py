...
from model import EAEFNet
#############################################################################################
parser = argparse.ArgumentParser(description='Test with pytorch')
#############################################################################################
parser.add_argument('--model_name', '-m', type=str, default='EAEF')
parser.add_argument('--weight_name', '-w', type=str, default='EAEF')
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        for it, (images, labels, names) in enumerate(test_loader):
            ...
            conf_total += conf
            # save demo images
            visualize(image_name=names, predictions=logit_mix.argmax(1), weight_name='Pred_' + args.weight_name)
