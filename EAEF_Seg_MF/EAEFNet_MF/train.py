        ...
        for it, (images, labels, names) in enumerate(val_loader):
            ...
            if accIter['val'] % 100 == 0:
                if view_figure:
                    ...
                    predicted_tensor = logits_T.argmax(1).unsqueeze(
...
def testing(epo, model, test_loader):
    ...
    with torch.no_grad():
        for it, (images, labels, names) in enumerate(test_loader):
            ...
            conf_total += conf
            #visualize(image_name=names, predictions=logit_mix.argmax(1), weight_name='Pred_'+str(epo))
...
def testing_s(epo, model, test_loader):
    ...
    print('saving testing results.')
    with open(testing_results_file, "r") as file:
        writer.add_text('testing_results_s', file.read().replace('\n', '  \n'), epo)
